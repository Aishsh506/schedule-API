...
namespace Web.Controllers
{
    ...
    public class BookingController : ControllerBase
    {
        ...
        [HttpGet, Route("moderation-list")]
        [Authorize(Roles = "Editor")]
        [ProducesResponseType(typeof(BookingRequestDTO[]), 200)]
        [ProducesResponseType(401)]
        [ProducesResponseType(403)]
        [ProducesResponseType(typeof(ErrorResponse), 500)]
        public IActionResult GetNewRequests()
        {
            try
            {
                return Ok(_bookingService.GetNewRequests());
            }
            catch(Exception ex)
            {
                return StatusCode(StatusCodes.Status500InternalServerError, new ErrorResponse(ex));
        ...
        [HttpPatch, Route("{id}/moderation/approve")]
        [Authorize(Roles = "Editor")]
        [ProducesResponseType(200)]
        ...
        public async Task<IActionResult> ApproveRequest([FromRoute] Guid id)
        {
            try
            {
                await _bookingService.ModerateRequest(id, true);
                return Ok();
            }
            catch (KeyNotFoundException)
            {
        ...
        [HttpPatch, Route("{id}/moderation/reject")]
        [Authorize(Roles = "Editor")]
        [ProducesResponseType(200)]
        [ProducesResponseType(400)]
        [ProducesResponseType(401)]
        ...
        public async Task<IActionResult> RejectRequest([FromRoute] Guid id)
        {
            try
            {
                await _bookingService.ModerateRequest(id, false);
                return Ok();
            }
            catch (KeyNotFoundException)
            {
